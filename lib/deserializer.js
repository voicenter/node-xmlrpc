...
Deserializer.prototype.onClosetag = function(el) {
  ...
  try {
    switch(el) {
      ...
      case 'STRING':
      case 'NAME':
        this.endString(data.replace(/\s/g, ''))
        break
      case 'ARRAY':
        this.endArray(data)
        break
...
Deserializer.prototype.endInt = function(data) {
  if(!data) {
    console.log("Got empty int data convert to 0",data);
    data = -1;
  }
...
Deserializer.prototype.endDouble = function(data) {
  if(!data){
    // added patch when Double is empty
    console.log("Got empty dubole data convert to 0",data);
    data = 0;
  }
